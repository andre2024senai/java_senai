...
Quando você *lê* uma pessoa do banco, o `id` já existe e precisa ser carregado.

**Onde essa classe aparece no projeto?**  
Em todo lugar. O DAO recebe e retorna objetos `Pessoa`. O `Main` cria objetos `Pessoa`. É o "tijolo" do sistema.
...
**`Connection` vem de onde?**  
Do pacote `java.sql.Connection`. O JDBC Driver (o `.jar` do MySQL) é quem implementa essa interface por baixo dos panos.
...
```
Main.java          → "quero inserir uma pessoa"
PessoaDAO.java     → sabe COMO inserir (escreve o SQL, usa JDBC)
...
public class Main {
    public static void main(String[] args) {
        ...
        try {
            dao.inserir(new Pessoa("Ana Silva", 22));
            dao.inserir(new Pessoa("Bruno Costa", 30));
...
1. **UPDATE e DELETE** — completar o CRUD no `PessoaDAO`
2. **Web com Spring Boot** — o Spring usa os mesmos conceitos, mas abstrai o JDBC com JPA/Hibernate
3. **Spring Data JPA** — você terá `findAll()`, `save()`, `deleteById()` prontos, sem escrever SQL na mão
